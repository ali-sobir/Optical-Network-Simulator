...
No_of_iterations =3;                           % Repeating the whole simulation for "No_of_iterations" times and then Averaging them.
...
LambdaArray = 20:20:100;                                                        % Average Arrival-rate (Poisson process)
...
for z= 1:No_of_iterations                                                       % Loop to repeat whole simulation 'No_of_iterations' times.
    ...
    for m= 1:length(LambdaArray)                                                % Loop to find Blocking Probability for different 'Lambda' values
       ...
       %% Main Simulation starts from here
        ...
        while blocked_requests < 1000                                           % To get good Blocking Probability estimate, atleast 100 connections should be blocked.
        ...
        % When Network is in steady state, we start counting No of events arrived in 'AH_Overall_request_number'.
                ...
                if Clock > 6*AverageHoldingTime                                 % when clock is greater than 6 times average holding time. 
        ...
        %%  Checking Resource's availability (FOR SPECTRUM ALLOCATION)
                ...
                [flag1, WavelengthIndexes] = ResourcesChecking(CurrentEvent.Bandwidth,NoOfNodes , Spectrum_Status, CurrentEvent.Route, Total_wavelengths);
